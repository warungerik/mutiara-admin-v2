...
<div class="overflow-hidden rounded-3xl flex flex-col md:flex-row bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-2xl">
    <div class="hidden md:block w-1/2 relative overflow-hidden border-r border-slate-100 dark:border-slate-800 min-h-[580px]">
        <div class="absolute inset-0 bg-linear-to-br from-blue-50 to-indigo-100 dark:from-slate-950 dark:to-indigo-950/40">
            <canvas id="dot-map-canvas" class="absolute inset-0 w-full h-full"></canvas>
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center p-8 z-10 text-center">
                ...
                <p class="text-xs text-slate-600 dark:text-slate-300 max-w-xs leading-relaxed">
                    Masuk untuk mengelola dashboard operasional, transaksi real-time, dan memantau analitik bisnis Anda.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="w-full md:w-1/2 p-8 md:p-10 flex flex-col justify-center bg-white dark:bg-slate-900">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 mb-6">Akses portal administrator MUTIARA ADMIN</p>

            <div class="mb-5">
                <button type="button" onclick="showToast('Fitur login Google terhubung di mode produksi.', 'info')"
            ...
            <div class="mb-4 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-[11px] text-slate-600 dark:text-slate-300 flex items-center justify-between">
                <span>Demo: <strong>admin@mutiara.io</strong></span>
                <button type="button" onclick="fillDemoCredentials()" class="text-indigo-600 dark:text-indigo-400 font-semibold hover:underline">
                    Isi Cepat
                </button>
            </div>
            ...
            <form action="{{ route('login.post') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label for="login-email" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Email <span class="text-indigo-600">*</span></label>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label for="login-password" class="block text-xs font-semibold text-slate-700 dark:text-slate-300">Sandi <span class="text-indigo-600">*</span></label>
                        <a href="{{ route('password.request') }}" class="text-[11px] text-indigo-600 dark:text-indigo-400 hover:underline">Lupa sandi?</a>
                    </div>
                    <div class="relative">
                        ...
                        <button type="button" onclick="togglePasswordVisibility('login-password', this)" aria-label="Tampilkan sandi" class="absolute right-3 top-2.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
                ...
                <div class="flex items-center">
                    <input type="checkbox" id="remember" name="remember" class="rounded-sm border-slate-300 dark:border-slate-700 text-indigo-600 focus:ring-indigo-500">
                    <label for="remember" class="ml-2 text-xs text-slate-600 dark:text-slate-400">Ingat saya</label>
                </div>

                <button type="submit" class="w-full py-2.5 px-4 rounded-xl text-xs font-semibold bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-md shadow-indigo-500/20 flex items-center justify-center gap-2 transition-all">
                    <span>Masuk</span>
            ...
            <div class="mt-5 pt-4 text-center border-t border-slate-100 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400">
                Belum punya akun?
                <a href="{{ route('register') }}" class="text-indigo-600 dark:text-indigo-400 font-semibold hover:underline">Daftar</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    function fillDemoCredentials() {
        document.getElementById('login-email').value = 'admin@mutiara.io';
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const canvas = document.getElementById('dot-map-canvas');
        if (!canvas) return;
